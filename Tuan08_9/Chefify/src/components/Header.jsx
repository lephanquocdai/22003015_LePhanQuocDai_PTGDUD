    ...
    <header className="sticky top-0 z-10 border-b border-zinc-200 bg-white/95 backdrop-blur-sm">
      <div className="mx-auto flex h-16 w-full max-w-[1280px] items-center gap-4 px-6">
        ...
        <div className="hidden max-w-xs flex-1 lg:block">
          <div className="flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1.5 focus-within:border-pink-500">
            ...
            <input
              ...
              onChange={(e) => onSearchResultsClick?.(e.target.value)}
        ...
        <nav className="hidden items-center gap-5 text-sm font-medium text-zinc-600 md:flex">
          <button 
            ...
            className={`hover:text-zinc-900 ${activePage === 'search' ? 'text-pink-600' : ''}`}
          >
            What to cook
          </button>
          <button 
            onClick={onRecipesClick} 
            className={`hover:text-zinc-900 ${activePage === 'subscription' ? 'text-pink-600' : ''}`}
          >
            Recipes
          </button>
          <button onClick={onSearchResultsClick} className="hover:text-zinc-900">
            Ingredients
          </button>
          <button onClick={onSearchResultsClick} className="hover:text-zinc-900">
            Occasions
          ...
          <div className="ml-auto flex items-center gap-3">
            <button
              ...
              className={`rounded-lg px-3 py-2 text-xs font-semibold ${
                isRecipeBoxPage ? 'bg-pink-100 text-pink-600' : 'bg-zinc-100 text-zinc-600'
              }`}
            >
              Your Recipe Box
            </button>
